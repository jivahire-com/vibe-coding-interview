...
<tbody>
  ...
  <tr><td><code>dependencies</code></td><td>object[] | object</td><td>no</td><td>Toolchain the candidate needs, shown (with a live ✓/✗ install check) in the extension's pre-clone dialog. <strong>Flat form:</strong> an array of <code>{"label": "CMake", "check": "cmake --version"}</code>. <strong>Nested form</strong> (richer, see <code>cpp-thread-safe-cache</code>): <code>{"toolchain": [{"name": "CMake", "min_version": "3.14", "check": "cmake --version", "install": {"macos": "...", "debian": "...", "windows": "..."}}], "auto_fetched": ["Catch2 v3 — fetched by CMake on first build"]}</code>. In both forms <code>check</code> must be a simple <code>&lt;tool&gt; &lt;flag&gt;</code> command (run without a shell). Omit for challenges that need no extra tooling.</td></tr>
